...
`--with-debug-intrinsics` on the command line, or by adding
...
the elaborated circuit and appends intrinsics as secret commands - user code is
not altered. Without the annotation/flag the phase is a no-op.
...
- **JDK 17+** restricts reflective access to non-public members. The pass calls
  ...
  use `--with-debug-intrinsics` only in development builds.
...
- **Nested constructor parameters are extracted recursively** up to a small
  depth limit. `toString` is invoked on user objects whose runtime class has
  no extractable accessors; arbitrary `toString` implementations therefore
  show up in the emitted metadata. The 256-byte cap bounds the *output size*
