...
/** Injects `circt_debug_*` secret-command intrinsics into an elaborated
  * [[Circuit]]. Driven by [[chisel3.stage.phases.AddDebugIntrinsics]].
  */
  ...
  private class ComponentDebugEmitter extends LazyLogging {
    ...
    private def createIntrinsic(target: Data, parent: Option[String], si: SourceInfo): Seq[Command] = {
      ...
      val subCmds: Seq[Command] = target match {
        ...
        case record: Record =>
          val pn = Some(signalRef(target))
          record.elements.values.flatMap(createIntrinsic(_, pn, si)).toSeq
        case vecLike: VecLike[_] =>
          val pn = Some(signalRef(target))
          vecLike.toSeq.flatMap(e => createIntrinsic(e.asInstanceOf[Data], pn, si))
        case _ => Nil
      }
